        ...
        <div class="row align-items-center">
          <div class="col-md-12 col-lg-5">
            <div class="hero-slider-content text-white">
              ...
              <div class="action-btns mt-3"><a href="#openings" class="btn secondary-solid-btn"><span class="blink"></span>
                  VIEW OPENINGS</a></div>
            </div>
          </div>
          ...
          <div class="row mt-5">
            ...
            <div class="tab col-md-4">
              @foreach ($designations as $des)
                <button class="tablinks" onClick="openCity(event, '{{ $des->designation_id }}')"
                  id="defaultOpen">{{ $des->getDesignation->designation }}</button>
              @endforeach
            </div>
                ...
                <div id="{{ $des->designation_id }}" class="tabcontent mb-3">
                  ...
                  @foreach ($vacancy as $row)
                    <div class="about-us-content-wrap white-bg border rounded p-4">
                      <h3><span class="color-secondary">{{ $row->title }}</span></h3>
                      <span class="heading-border mb-4"></span>

                      <p class="mb-2"><strong>No. of Position :</strong> {{ $row->no_of_position }} &nbsp; &nbsp; &nbsp;
          ...
          <div class="row mt-3">
            <div class="col-lg-12">
              <div id="accordion-1" class="accordion accordion-faq">
                @foreach ($designations as $des)
                  <div class="card">
                    <div class="card-header py-3 collapsed" id="heading-1-{{ $des->designation_id }}"
                      data-toggle="collapse" role="button" data-target="#collapse-1-{{ $des->designation_id }}"
                      aria-expanded="false" aria-controls="collapse-1-{{ $des->designation_id }}">
        ...
        <div class="row align-items-center">
          <div class="col-md-8">
            <div class="contact-us-form gray-light-bg rounded p-4">
              <form action="{{ url('apply-job') }}" method="POST" class="login-signup-form"
                    ...
                    <div class="form-group">
                      ...
                      <div class="input-group input-group-merge">
                        ...
                        <select name="experience" id="experience" class="form-control">
                          ...
                          @endphp
                          @foreach ($exp as $exp)
                            <option value="{{ $exp }}" {{ $exp == old('experience') ? 'selected' : '' }}>
                              {{ $exp }}</option>
                          @endforeach
                        </select>
                      </div>
                        ...
                        <select name="position" id="position" class="form-control">
                          ...
                          @foreach ($designations as $des)
                            <option value="{{ $des->designation_id }}"
                              {{ $des->designation_id == old('position') ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                      ...
                      <div class="input-group input-group-merge">
                        ...
                        <textarea maxlength="1000" name="msg" id="msg" type="text" class="form-control"
                          ...
                          style="min-height:49px; padding-top:17px;     padding-left: 40px; text-indent: 5px;"></textarea>
                      </div>
                      <span class="text-danger">
                        @error('msg')
                          {{ $message }}
